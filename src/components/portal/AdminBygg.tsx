...
type FaseTilstand = "ferdig" | "aktiv" | "feil";

/**
 * Tidslinjen drives av selve loggen (som nullstilles per kjøring), så den
 * viser KUN gjeldende bygg — også ved revisjon, der stegene er andre enn
 * et førstebygg. Siste linje er «aktiv» mens bygget jobber, «feil» hvis det
 * stoppet/feilet, ellers er alt «ferdig».
 */
function loggTilstand(
  index: number,
  antall: number,
  status: ByggStatus
): FaseTilstand {
  if (index < antall - 1) return "ferdig";
  if (AKTIV.has(status)) return "aktiv";
  if (status === "feilet" || status === "stoppet") return "feil";
  return "ferdig";
}
...
export default function AdminBygg({ kartleggingId, kartStatus }: AdminByggProps) {
  ...
  return (
    <div className="vk-bygg">
      ...
      {/* Log-drevet tidslinje — viser kun gjeldende kjøring (loggen
          nullstilles per bygg/revisjon). Siste linje pulser mens den jobber. */}
      {logg.length > 0 ? (
        <ol className="vk-bygg-faser" aria-label="Byggesteg" aria-live="polite">
          {logg.map((l, i) => {
            const tilstand = loggTilstand(i, logg.length, status);
            return (
              <li
                key={`${l.tid}-${i}`}
                className={`vk-bygg-fase vk-bygg-fase--${tilstand}`}
              >
                <span className="vk-bygg-fase-merke" aria-hidden="true">
                  {tilstand === "ferdig" ? "✓" : tilstand === "feil" ? "✕" : "●"}
                </span>
                <span className="vk-bygg-fase-navn">{l.melding}</span>
                <span className="vk-mono vk-bygg-fase-na">{formatTid(l.tid)}</span>
              </li>
            );
          })}
        </ol>
